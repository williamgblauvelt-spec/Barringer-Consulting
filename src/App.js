...
.section-pad-sm { padding: 60px 48px; }
...
.grid-2-card { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; }
.grid-contact { display: grid; grid-template-columns: 1fr 2fr; gap: 80px; }
...
@media (max-width: 768px) {
    ...
    .grid-2-start { grid-template-columns: 1fr; gap: 40px; }
    .grid-2-card { grid-template-columns: 1fr; gap: 20px; }
    .grid-contact { grid-template-columns: 1fr; gap: 40px; }
    .grid-form-row { grid-template-columns: 1fr; gap: 0; margin-bottom: 0; }
    ...
    .service-order-1 { order: 1; }
    .service-order-2 { order: 1; }
    .lead-form-wrap { max-width: 100% !important; }
}
`

function LeadForm({ bg = G.white }) {
    ...
    async function handleSubmit(e) {
        ...
        setStatus("sending")
        try {
            const res = await fetch("https://usebasin.com/f/3028f42e011a", {
                method: "POST",
    ...
    return (
        <div className="lead-form-wrap" style={{ background: bg, borderRadius: 8, padding: "40px 36px", border: `1px solid ${G.gray200}`, maxWidth: 520, width: "100%" }}>
            <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 26, fontWeight: 600, color: G.green800, marginBottom: 6 }}>
                Start the Conversation
            </p>
            <p style={{ fontSize: 14, color: G.gray500, marginBottom: 28, lineHeight: 1.6 }}>
                Tell us where you are. We'll tell you where you could be.
            </p>

            {status === "success" ? (
                <div style={{ textAlign: "center", padding: "40px 0" }}>
                    <div style={{ width: 48, height: 48, borderRadius: "50%", background: G.green100, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M5 13l4 4L19 7" stroke={G.green700} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                    </div>
                    <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontWeight: 600, color: G.green800, marginBottom: 8 }}>We'll be in touch.</p>
                    <p style={{ fontSize: 14, color: G.gray500 }}>Expect a response within one business day.</p>
                </div>
            ) : (
                <form onSubmit={handleSubmit}>
                    <div className="grid-form-row">
                        <div>
                            ...
                            <input style={inputStyle} type="text" placeholder="John Smith" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required />
                        </div>
                        <div>
                    ...
                    <div style={{ marginBottom: 16 }}>
                        ...
                        <input style={inputStyle} type="email" placeholder="john@company.com" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} required />
                    </div>
                    <div style={{ marginBottom: 24 }}>
                        <label style={labelStyle}>What are you working on?</label>
                        <textarea style={{ ...inputStyle, height: 100, resize: "vertical" }} placeholder="Tell us about your current sales situation and what you're hoping to improve..." value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} />
                    </div>
                    <button type="submit" disabled={status === "sending"} style={{ width: "100%", padding: "15px 24px", background: G.green800, color: G.white, border: "none", borderRadius: 4, fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", cursor: status === "sending" ? "wait" : "pointer" }}>
...
function Nav({ page, setPage }) {
    ...
    const navBtnStyle = (key) => ({
        ...
        color: page === key ? G.white : "rgba(255,255,255,0.6)",
        borderBottom: page === key ? `2px solid ${G.gold}` : "2px solid transparent",
        paddingBottom: 2,
        transition: "all 0.2s",
    })
    ...
    return (
        <nav style={{ position: "sticky", top: 0, zIndex: 100, background: G.green900, borderBottom: `1px solid rgba(255,255,255,0.08)` }}>
            <div className="nav-pad" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 72 }}>
                <button onClick={() => navigate("home")} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 32, height: 32, background: G.green700, borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <span style={{ color: G.white, fontFamily: "'Cormorant Garamond', serif", fontWeight: 700, fontSize: 16 }}>B</span>
                    </div>
                    <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, fontWeight: 600, color: G.white, letterSpacing: "0.02em" }}>
                        Barringer Consulting
                    </span>
                </button>

                <div className="nav-links">
                    {[["home", "Home"], ["services", "Services"]].map(([key, label]) => (
                ...
                <button className="hamburger" onClick={() => setMenuOpen(!menuOpen)} aria-label="Menu">
                    <span style={{ width: 22, height: 2, background: G.white, display: "block", transition: "all 0.2s", transform: menuOpen ? "rotate(45deg) translate(5px, 5px)" : "none" }}/>
                    <span style={{ width: 22, height: 2, background: G.white, display: "block", opacity: menuOpen ? 0 : 1, transition: "all 0.2s" }}/>
                    <span style={{ width: 22, height: 2, background: G.white, display: "block", transition: "all 0.2s", transform: menuOpen ? "rotate(-45deg) translate(5px, -5px)" : "none" }}/>
                </button>
            </div>
...
function HomePage({ setPage }) {
    ...
    return (
        <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <section style={{ background: G.green800, borderBottom: `1px solid ${G.green700}` }}>
                <div className="nav-pad">
                    <div className="stat-bar" style={{ maxWidth: 1100, margin: "0 auto" }}>
                        {[["79%", "Average increase in order size"], ["90 days", "Typical time to measurable results"], ["1:1", "Dedicated consulting, not group coaching"]].map(([stat, label]) => (
                            <div key={stat} style={{ padding: "32px 24px", textAlign: "center" }}>
                                <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 42, fontWeight: 700, color: G.white, lineHeight: 1 }}>{stat}</p>
                                <p style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", marginTop: 8, letterSpacing: "0.04em" }}>{label}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section className="section-pad" style={{ background: G.white }}>
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    <div className="grid-2">
                        <div>
                            <p style={{ fontSize: 12, fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", color: G.green700, marginBottom: 16 }}>Our Approach</p>
                            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(28px, 4vw, 42px)", fontWeight: 600, color: G.gray900, lineHeight: 1.15, marginBottom: 24 }}>
                                Performance isn't about working harder. It's about selling smarter.
                            </h2>
                            <p style={{ fontSize: 16, lineHeight: 1.8, color: G.gray500, marginBottom: 20 }}>
                                Most sales training focuses on scripts and tactics. We focus on the underlying mechanics — why buyers say yes, how to position value, and how to close without leaving revenue on the table.
                            </p>
                            <p style={{ fontSize: 16, lineHeight: 1.8, color: G.gray500, marginBottom: 36 }}>
                                We work with you directly. No cohorts, no generic frameworks, no one-size-fits-all programs. Every engagement is built around your specific deals, your specific pipeline, and what's actually holding you back.
                            </p>
                            <button onClick={() => navigate("services")} style={{ padding: "13px 28px", background: "transparent", color: G.green800, border: `1px solid ${G.green800}`, borderRadius: 4, fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", cursor: "pointer" }}>
                                See How We Work
                            </button>
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                            {[["Diagnose", "We audit your current approach — pricing conversations, objection handling, and deal structure — to find the gaps."], ["Rebuild", "We rebuild your positioning, your value narrative, and your close strategy around what buyers actually respond to."], ["Execute", "We work alongside you on real deals in real time, so the improvements stick."]].map(([title, desc], i) => (
                                <div key={title} style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
                                    <div style={{ width: 36, height: 36, borderRadius: "50%", background: G.green100, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                        <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 16, fontWeight: 700, color: G.green800 }}>{i + 1}</span>
                                    </div>
                                    <div>
                                        <p style={{ fontWeight: 500, color: G.gray900, marginBottom: 6 }}>{title}</p>
                                        <p style={{ fontSize: 14, lineHeight: 1.7, color: G.gray500 }}>{desc}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <section className="section-pad" style={{ background: G.gray50, borderTop: `1px solid ${G.gray200}` }}>
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    <div style={{ textAlign: "center", marginBottom: 64 }}>
                        <p style={{ fontSize: 12, fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", color: G.green700, marginBottom: 12 }}>Services</p>
                        <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(26px, 4vw, 42px)", fontWeight: 600, color: G.gray900 }}>Built for sales professionals who are serious about growth</h2>
                    </div>
                    <div className="grid-2-card">
                        {[
                            { title: "Sales Consulting", tag: "1:1 Engagement", desc: "A dedicated engagement focused on diagnosing and fixing the specific mechanics holding back your revenue. We go deep on your deals, your pipeline, and your positioning.", items: ["Pipeline audit", "Deal strategy sessions", "Close rate improvement", "Revenue per deal optimization"] },
                            { title: "Sales Training", tag: "For Individuals", desc: "A structured program for sales professionals who want to sharpen foundational skills — value framing, objection handling, and negotiation — and see immediate results in the field.", items: ["Value-based selling", "Objection handling", "Price & negotiation", "Buyer psychology"] },
                        ].map(svc => (
                            <div key={svc.title} style={{ background: G.white, border: `1px solid ${G.gray200}`, borderTop: `3px solid ${G.green700}`, borderRadius: 8, padding: "40px 36px" }}>
                                <span style={{ fontSize: 11, fontWeight: 500, letterSpacing: "0.1em", textTransform: "uppercase", color: G.green700, background: G.green50, padding: "4px 10px", borderRadius: 20 }}>{svc.tag}</span>
                                <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 28, fontWeight: 600, color: G.gray900, margin: "16px 0 12px" }}>{svc.title}</h3>
                                <p style={{ fontSize: 15, lineHeight: 1.7, color: G.gray500, marginBottom: 24 }}>{svc.desc}</p>
                                <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 10 }}>
                                    {svc.items.map(item => (
                                        <li key={item} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 14, color: G.gray700 }}>
                                            <div style={{ width: 6, height: 6, borderRadius: "50%", background: G.green600, flexShrink: 0 }}/>
                                            {item}
                                        </li>
                                    ))}
                                </ul>
                    ...
                    <div style={{ textAlign: "center", marginTop: 40 }}>
                        <button onClick={() => navigate("services")} style={{ padding: "14px 32px", background: G.green800, color: G.white, border: "none", borderRadius: 4, fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", cursor: "pointer" }}>
                            Learn More
                        </button>
                    </div>
                </div>
            ...
            <section className="section-pad" style={{ background: G.green900 }}>
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    <div className="grid-2">
                        <div>
                            <p style={{ fontSize: 12, fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", color: G.gold, marginBottom: 16 }}>Get Started</p>
                            ...
                            <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(255,255,255,0.6)" }}>
                                The first conversation is free. Tell us where you are, and we'll tell you what's possible.
                            </p>
                        </div>
                        <LeadForm bg={G.white}/>
                    </div>
    ...
    return (
        <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <section className="hero-pad" style={{ background: G.green900, paddingBottom: 80 }}>
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    <p style={{ fontSize: 12, fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", color: G.gold, marginBottom: 16 }}>Services</p>
                    <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(32px, 5vw, 60px)", fontWeight: 600, color: G.white, maxWidth: 640, lineHeight: 1.15, marginBottom: 24 }}>
                        Serious about sales. Serious about results.
                    </h1>
                    <p style={{ fontSize: 18, lineHeight: 1.7, color: "rgba(255,255,255,0.6)", maxWidth: 520 }}>
                        Every engagement is built around your deals and your goals. We don't do generic programs. We do work that moves numbers.
                    </p>
                </div>
            </section>
            ...
            <section className="section-pad" style={{ background: G.white }}>
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    <div className="grid-2-start">
                        <div>
                            <span style={{ fontSize: 11, fontWeight: 500, letterSpacing: "0.1em", textTransform: "uppercase", color: G.green700, background: G.green50, padding: "4px 12px", borderRadius: 20 }}>1:1 Engagement</span>
                            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(28px, 4vw, 42px)", fontWeight: 600, color: G.gray900, margin: "20px 0 16px", lineHeight: 1.15 }}>Sales Consulting</h2>
                            <p style={{ fontSize: 16, lineHeight: 1.8, color: G.gray500, marginBottom: 20 }}>
                                A focused, high-touch engagement for sales professionals who know something isn't working and want to fix it precisely. Not a course. Not a workshop. A working relationship built around your actual pipeline.
                            </p>
                            <p style={{ fontSize: 16, lineHeight: 1.8, color: G.gray500, marginBottom: 36 }}>
